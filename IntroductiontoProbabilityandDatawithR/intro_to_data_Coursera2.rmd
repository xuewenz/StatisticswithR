...
sfo_feb_flights %>% group_by(carrier) %>% summarise(mean_dd = mean(arr_delay), sd_dd = sd(arr_delay), median_dd = median(arr_delay), IQR_dd = IQR(arr_delay), n = n()) %>% arrange(desc(IQR_dd))
...
nycflights %>% summarise(avg_speed = avg_speed, tailnum = tailnum) %>% arrange(desc(avg_speed)) %>% head(n=5)
```
